...
for k in range(10):
    ...
    sim = (Simulation().
        ...
        add([
            ...
            Group(m=10**k, attr={ 'is-migrating': False }, rel={ Site.AT: site_conflict })
        ]).
        run(4*12)  # months
    )
    ...
    if persistence:
        series = [
            { 'var': 'migrating_m', 'lw': 2, 'linestyle': '-',  'marker': '', 'color': 'blue',  'markersize': 0, 'lbl': 'Migrating' },
            { 'var': 'settled_m',   'lw': 2, 'linestyle': '--', 'marker': '', 'color': 'orange', 'markersize': 0, 'lbl': 'Settled'   },
            { 'var': 'dead_m',      'lw': 2, 'linestyle': ':',  'marker': '', 'color': 'green',   'markersize': 0, 'lbl': 'Dead'      }
        ]
        fig = sim.probes[0].plot(series, ylabel='Population mass', xlabel='Iteration (month from start of conflict)', figsize=(8, 6), subplot_b=0.15)
        fig.suptitle(f'Migration PRAM Model - {48} iterations at mass 10^{k}', y=0.95)
        fig.savefig(f"Migration_PRAM_out_10_pow_{k}")
